!function (e) {
  if ('object' == typeof exports)
    ...
    var f;
    'undefined' != typeof window ? f = window : 'undefined' != typeof global ? f = global : 'undefined' != typeof self && (f = self), f.model = e();
  }
}(function () {
 ...
 */
        function typeOf(o) {
          ...
          var s = Object.prototype.toString.call(o).match(/^\[object (.*)\]$/)[1].toLowerCase(), supDataTypes = [
              'array',
              'string',
              'function',
              'number',
 ...
 */
        function extend(source, defaults) {
          for (var property in defaults) {
            if (defaults[property] && defaults[property].constructor && defaults[property].constructor === Object) {
 ...
 */
        function inherits(descendant, parent, parentName) {
          var sConstructor = parent.constructor.toString().toLowerCase(), aMatch = sConstructor.match(/\s*function (.*)\(/);
          if (aMatch !== null) {
 ...
 */
        function filterObject(originalObject, listOfFilterKeys, included) {
          ...
          for (var key in originalObject) {
            if (originalObject.hasOwnProperty(key) && (!included && (!listOfFilterKeys || listOfFilterKeys.indexOf(key) === -1) || included && listOfFilterKeys && listOfFilterKeys.indexOf(key) !== -1)) {
 ...
 */
    ...
    2: [
      function (_dereq_, module, exports) {
        'use strict';
        var Canvas = _dereq_('./canvas'), Timeline = _dereq_('./timeline'), MediaTimeline = _dereq_('./mediaTimeline'), JsonSerializer = _dereq_('./serialization/jsonSerializer'), Common = _dereq_('animates-common');
        /**
 ...
 */
        function Animation(options) {
          ...
          var _self = this,
            ...
            updateObservers = {}, loadCompleteObservers = {};
          /**
	*  Constructor
	*/
          (function preInit() {
          ...
          function applyShapeCreateOperation(opParams) {
            var mediaTimeline, result = getResultObject();
          ...
          function applyShapeRemoveOperation(opParams) {
            var mediaTimeline = new MediaTimeline(), result = getResultObject();
          ...
          function applyEffectCreateOperation(opParams) {
            var mediaTimeline = _self.timeline.getMediaTimeline(opParams.mediaObjectId), result = getResultObject();
            if (mediaTimeline) {
          ...
          function applyMediaFrameUpdateOperation(opParams) {
            var mediaTimeline = _self.timeline.getMediaTimeline(opParams.mediaObjectId), newProperty, updateResult, notUpdatedProperties, result = getResultObject();
            if (mediaTimeline) {
          ...
          this.applyOperation = function applyOperation(target, operation, opParams, context) {
            var result = getResultObject(), params;
	...
	*/
      ...
      },
      {
        './canvas': 3,
        './mediaTimeline': 18,
        './serialization/jsonSerializer': 30,
        './timeline': 37,
        'animates-common': 1
      }
    ],
 ...
 */
	...
	*/
    ...
    4: [
      function (_dereq_, module, exports) {
        'use strict';
        var Common = _dereq_('animates-common'), PropertyBuilder = _dereq_('./properties/propertyBuilder'), CompositePropertyBuilder = _dereq_('./properties/compositePropertyBuilder'), JsonSerializer = _dereq_('./serialization/jsonSerializer');
        /**
 ...
 */
        function Effect(options, builder) {
          options = options || {};
          var _self = this, guid = '', propBuilder, defaultOptions = {}, currentOptions = {};
          /**
	 *  Constructor
	 */
          (function preInit() {
	...
	*/
          this.getOptions = function getOptions() {
            return currentOptions.valuesToJSON();  // TODO esto no va mas
          };
          /**
	...
	*/
      ...
      {
        './properties/compositePropertyBuilder': 23,
        './properties/propertyBuilder': 27,
        './serialization/jsonSerializer': 30,
        'animates-common': 1
      }
    ],
    5: [
      function (_dereq_, module, exports) {
        'use strict';
        var Common = _dereq_('animates-common'), JsonSerializer = _dereq_('../serialization/jsonSerializer'), PropertyBuilder = _dereq_('../properties/propertyBuilder'), DictionaryPropertyBuilder = _dereq_('../properties/dictionaryPropertyBuilder'), CompositePropertyBuilder = _dereq_('../properties/compositePropertyBuilder'), straightPathStrategy = _dereq_('./pathStrategies/straightPathStrategy'), Effect = _dereq_('../finiteEffect.js');
        /**
 *  Creates a new FadeEffect.
 *  @class Represents an FadeEffect .
 */
        function FadeEffect(options, builder) {
          var _self = this, propBuilder, defaultOptions = {
              'startOpacity': 0,
              'endOpacity': 1
            };
          /**
	 ...
	 */
          (function preInit() {
            ...
            propBuilder.property('startOpacity', PropertyBuilder).value(options.startOpacity).type('float').add().property('endOpacity', PropertyBuilder).value(options.endOpacity).type('float').add();
	 ...
	 */
          this.getProperties = function (tick, mediaFrameProperties) {
            var startTick = _self.getOption('startTick'), endTick = _self.getOption('endTick'), startOpacity = _self.getOption('startOpacity'), endOpacity = _self.getOption('endOpacity'), currentOpacity;
            if (startTick > tick) {
              return mediaFrameProperties;
            }
            if (endTick <= tick) {
              ...
              var ticksAmount = endTick - startTick, currentPathPercentage = (tick - startTick) / ticksAmount, delta = (endOpacity - startOpacity) * currentPathPercentage;
          ...
          this.updateProperties = function (tick, updatedProperties) {
            var startTick = _self.getOption('startTick'), endTick = _self.getOption('endTick'), opacity = updatedProperties.opacity, changedProperties = [];
	...
	*/
      ...
      },
      {
        '../finiteEffect.js': 15,
        '../properties/compositePropertyBuilder': 23,
        '../properties/dictionaryPropertyBuilder': 25,
        '../properties/propertyBuilder': 27,
        '../serialization/jsonSerializer': 30,
        './pathStrategies/straightPathStrategy': 13,
        'animates-common': 1
      }
    ],
    6: [
 ...
 */
	 ...
	 */
          (function preInit() {
            propBuilder = builder || new CompositePropertyBuilder();
            options = Common.extend(options || {}, defaultOptions);
            propBuilder.property('path', PropertyBuilder).value(options.path).type('string').strictValues(['Straight']).add().property('startPosition', CompositePropertyBuilder).property('x', PropertyBuilder).value(options.startPosition.x).type('float').add().property('y', PropertyBuilder).value(options.startPosition.y).type('float').add().add().property('endPosition', CompositePropertyBuilder).property('x', PropertyBuilder).value(options.endPosition.x).type('float').add().property('y', PropertyBuilder).value(options.endPosition.y).type('float').add().add();
	 ...
	 */
          ...
          this.updateProperties = function (tick, updatedProperties) {
            var startTick = _self.getOption('startTick'), endTick = _self.getOption('endTick'), positionX = updatedProperties['position.x'], positionY = updatedProperties['position.y'], changedProperties = [];
	...
	*/
    ...
    7: [
      function (_dereq_, module, exports) {
        'use strict';
        var Common = _dereq_('animates-common'), JsonSerializer = _dereq_('../serialization/jsonSerializer'), PropertyBuilder = _dereq_('../properties/propertyBuilder'), DictionaryPropertyBuilder = _dereq_('../properties/dictionaryPropertyBuilder'), CompositePropertyBuilder = _dereq_('../properties/compositePropertyBuilder'), MultiPointScaleEffect = _dereq_('./multiPointScaleEffect.js'), segmentHelper = _dereq_('./utils/segmentHelper');
        /**
 ...
 */
        function MultiPointFontSizeScaleEffect(options, builder) {
          var _self = this, defaultOptions = {}, currentOptions;
          /**
	 *  Constructor
	 */
          (function preInit() {
            var pointsSchemaBuilder, propBuilder = builder || new CompositePropertyBuilder();
            currentOptions = Common.extend(options || {}, defaultOptions);
            // Build points schema
            pointsSchemaBuilder = new CompositePropertyBuilder();
            pointsSchemaBuilder.property('fontSize', PropertyBuilder).type('float').constraint(function (val) {
	...
	*/
      ...
      },
      {
        ...
        '../properties/propertyBuilder': 27,
        '../serialization/jsonSerializer': 30,
        './multiPointScaleEffect.js': 11,
        './utils/segmentHelper': 14,
        'animates-common': 1
      }
    ],
    8: [
 ...
 */
	 ...
	 */
          (function preInit() {
            ...
            propBuilder = builder || new CompositePropertyBuilder();
            currentOptions = Common.extend(options || {}, defaultOptions);
            propBuilder.property('path', PropertyBuilder).value(currentOptions.path).type('string').strictValues(['Straight']).add();
            // Build points schema
            pointsSchemaBuilder = new CompositePropertyBuilder();
            pointsSchemaBuilder.property('position', CompositePropertyBuilder).property('x', PropertyBuilder).type('float').add().property('y', PropertyBuilder).type('float').add().add();
	 ...
	 */
          ...
          this.updateProperties = function (tick, updatedProperties) {
            var positionX = updatedProperties['position.x'], positionY = updatedProperties['position.y'], changedProperties = [], points;
            if (positionX === undefined && positionY === undefined) {
              return { 'updatedProperties': changedProperties };
            }
 ...
 */
	 ...
	 */
          (function preInit() {
            ...
            // Build points schema
            pointsSchemaBuilder = new CompositePropertyBuilder();
            pointsSchemaBuilder.property('radius', PropertyBuilder).type('float').constraint(function (val) {
              return val === undefined || val >= 0;
            }).add();
            _self.MultiPointScaleEffect(currentOptions, propBuilder, pointsSchemaBuilder);
          }());
	...
	*/
    ...
    10: [
      function (_dereq_, module, exports) {
        'use strict';
        var Common = _dereq_('animates-common'), JsonSerializer = _dereq_('../serialization/jsonSerializer'), PropertyBuilder = _dereq_('../properties/propertyBuilder'), DictionaryPropertyBuilder = _dereq_('../properties/dictionaryPropertyBuilder'), CompositePropertyBuilder = _dereq_('../properties/compositePropertyBuilder'), straightPathStrategy = _dereq_('./pathStrategies/straightPathStrategy'), segmentHelper = _dereq_('./utils/segmentHelper'), MultiPointEffect = _dereq_('../multiPointEffect.js');
        /**
 ...
 */
        function MultiPointRotateEffect(options, builder) {
          var _self = this, propBuilder, defaultOptions = { 'points': {} }, currentOptions;
          /**
	 *  Constructor
	 */
          (function preInit() {
            ...
            // Build points schema
            pointsSchemaBuilder = new CompositePropertyBuilder();
            pointsSchemaBuilder.property('angle', PropertyBuilder).type('float').add().property('motion', PropertyBuilder).type('string').value('clockwise').strictValues([
              'clockwise',
              'counter-clockwise'
            ]).add();
            _self.MultiPointEffect(currentOptions, propBuilder, pointsSchemaBuilder);
          ...
          function getAngleFor(currentTick, startPoint, endPoint, clockwise) {
            var startTick = startPoint.tick, endTick = endPoint.tick, startAngle = startPoint.angle, endAngle = endPoint.angle, resultAngle, newLoopTick;
	 ...
	 */
          this.getProperties = function (tick, mediaFrameProperties) {
            var points = _self.getPointsArray(), segment = segmentHelper.getSegment(tick, points);
          ...
          this.updateProperties = function (tick, updatedProperties) {
            var angle = updatedProperties.angle, motion = updatedProperties.motion, changedProperties = [], points;
            ...
            if (newPoint && newPoint.target === _self.getGuid()) {
              ...
              return {
                'updatedProperties': [
                  'angle',
                  'motion',
                  'MultiPointRotateEffect.newPoint'
                ]
              };
            }
            points = _self.getOption('points');
	...
	*/
    ...
    11: [
      function (_dereq_, module, exports) {
        'use strict';
        var Common = _dereq_('animates-common'), JsonSerializer = _dereq_('../serialization/jsonSerializer'), PropertyBuilder = _dereq_('../properties/propertyBuilder'), DictionaryPropertyBuilder = _dereq_('../properties/dictionaryPropertyBuilder'), CompositePropertyBuilder = _dereq_('../properties/compositePropertyBuilder'), MultiPointEffect = _dereq_('../multiPointEffect.js'), segmentHelper = _dereq_('./utils/segmentHelper');
        /**
 ...
 */
	 ...
	 */
          (function preInit() {
            var propBuilder = builder || new CompositePropertyBuilder(), pointsBuilder = pointsSchemaBuilder || new CompositePropertyBuilder();
            currentOptions = Common.extend(options || {}, defaultOptions);
          ...
          function getScaleFor(currentTick, startPoint, endPoint, scaleType) {
            var startTick = startPoint.tick, endTick = endPoint.tick, startScale = startPoint[scaleType], endScale = endPoint[scaleType];
            ...
            var ticksAmount = endTick - startTick, currentPathPercentage = (currentTick - startTick) / ticksAmount, delta = (endScale - startScale) * currentPathPercentage;
	 ...
	 */
          this.getProperties = function (tick, mediaFrameProperties) {
            var points = _self.getPointsArray(), segment = segmentHelper.getSegment(tick, points), scalableProperties = getScalablePropertiesIfExists();
          ...
          function addScalePoint(guid, tick, inputData) {
            var points = _self.getPointsArray(), segment = segmentHelper.getSegment(tick, points), scalableProperties = getScalablePropertiesIfExists(), data = {};
            applyToScalableProperties(function (property) {
          ...
          this.updateProperties = function (tick, updatedProperties) {
            var scalableProperties = getScalablePropertiesIfExists(), changedProperties = [], data = {}, points, notFound = true;
            applyToScalableProperties(function (property) {
            ...
            return {
              ...
              'newProperties': {
                'MultiPointScaleEffect.newPoint': {
                  'guid': newPointGuid,
                  'target': _self.getGuid()
                }
              }
 ...
 */
	 ...
	 */
          (function preInit() {
            ...
            // Build points schema
            pointsSchemaBuilder = new CompositePropertyBuilder();
            pointsSchemaBuilder.property('height', PropertyBuilder).type('float').constraint(function (val) {
              return val === undefined || val >= 0;
            }).add().property('width', PropertyBuilder).type('float').constraint(function (val) {
              return val === undefined || val >= 0;
            }).add();
            _self.MultiPointScaleEffect(currentOptions, propBuilder, pointsSchemaBuilder);
          }());
 ...
 */
	 ...
	 */
          function getPositionFor(currentTick, startPoint, endPoint) {
            var startX = startPoint.position.x, startY = startPoint.position.y, startTick = startPoint.tick, endX = endPoint.position.x, endY = endPoint.position.y, endTick = endPoint.tick;
            if (startTick > currentTick) {
            ...
            var ticksAmount = endTick - startTick, currentPathPercentage = (currentTick - startTick) / ticksAmount, xDelta = (endX - startX) * currentPathPercentage, yDelta = (endY - startY) * currentPathPercentage;
            return {
              'x': startX + Math.round(xDelta),
              'y': startY + Math.round(yDelta)
            };
          }
 ...
 */
        function getSegment(currentTick, points) {
          ...
          if (points[0].tick > currentTick) {
            return { endPoint: points[0] };
          }
          if (points[points.length - 1].tick <= currentTick) {
            return { startPoint: points[points.length - 1] };
          }
    ...
    15: [
      function (_dereq_, module, exports) {
        'use strict';
        var Common = _dereq_('animates-common'), PropertyBuilder = _dereq_('./properties/propertyBuilder'), CompositePropertyBuilder = _dereq_('./properties/compositePropertyBuilder'), JsonSerializer = _dereq_('./serialization/jsonSerializer'), Effect = _dereq_('./effect');
        /**
 ...
 */
	 ...
	 */
          (function preInit() {
            ...
            propBuilder = builder || new CompositePropertyBuilder();
            propBuilder.property('endTick', PropertyBuilder).value(currentOptions.endTick).type('float').constraint(function (val) {
              return val >= 0 && (!_self.getOption || val > _self.getOption('startTick'));
            }).add().property('startTick', PropertyBuilder).value(currentOptions.startTick).type('float').constraint(function (val) {
              return val >= 0 && (!_self.getOption || val < _self.getOption('endTick'));
            }).add();
	...
	*/
    ...
    16: [
      function (_dereq_, module, exports) {
        'use strict';
        var Common = _dereq_('animates-common'), MediaObject = _dereq_('./mediaObject');
        /**
 ...
 */
        function MediaFrame(options) {
          var _self = this, defaultOptions = {
              currentTick: -1,
              mediaObject: null
            }, currentOptions, currentProperties;
          /**
	*  Constructor
	*/
          (function preInit() {
            currentOptions = Common.extend(options || {}, defaultOptions), currentProperties = currentOptions.mediaObject.getProperties();
          }());
          /**
	 ...
	 */
          this.getProperty = function getProperty(name) {
            if (name) {
              var parts = name.split('.'), parent = currentProperties, currentPart = '', length = parts.length;
 ...
 */
        function MediaObject(options, builder) {
          var _self = this, guid = '', propBuilder, properties, defaultOptions = { name: 'Object' };
          /**
	*  Constructor
	*/
          (function preInit() {
            ...
            propBuilder.property('name', PropertyBuilder).type('string').value(options.name).add();
	 ...
	 */
          this.getProperties = function getProperties() {
            return properties.valuesToJSON();  // TODO esto no va mas
          };
          /**
	...
	*/
    ...
    18: [
      function (_dereq_, module, exports) {
        'use strict';
        var MediaFrame = _dereq_('./mediaFrame'), JsonSerializer = _dereq_('./serialization/jsonSerializer'), Common = _dereq_('animates-common');
        /**
 ...
 */
        function MediaTimeline(options) {
          ...
          var _self = this,
            // Save the this reference for later use
            mediaObject = options.mediaObject, effects = {};
          /**
	*  Constructor
	*/
          (function preInit() {
	 ...
	 */
          this.getMediaFrameFor = function getMediaFrameFor(currentTick) {
            var mediaObjectFrame = new MediaFrame({
                'mediaObject': mediaObject,
                'currentTick': currentTick
              }), effectsArray = [], currentEffect;
            ...
            for (var i = effectsArray.length - 1; i >= 0; i--) {
              ...
              if (currentEffect.isInfinite()) {
                ...
                if (currentTick >= currentEffect.getOption('startTick')) {
                  var effectEndTick = currentEffect.getOption('endTick'), endTick = currentTick < effectEndTick ? currentTick : effectEndTick;
                  mediaObjectFrame.properties(currentEffect.getProperties(endTick, mediaObjectFrame.properties()));
                }
              }
            }
	...
	*/
	 ...
	 */
          this.getEffectsForTick = function (tick) {
            var effects = this.getEffects(), resultEffects = [];
            for (var id in effects) {
              if (effects.hasOwnProperty(id)) {
                var currentEffect = effects[id];
	...
	*/
          this.getEffectsForTickThatMatch = function (tick, propertiesList) {
            var effects = this.getEffectsForTick(tick), resultEffects = [];
	...
	*/
          this.updateEffectsThatMatch = function (tick, updatedProperties) {
            var propertiesList = Common.getKeysFromObject(updatedProperties), effects = this.getEffectsForTickThatMatch(tick, propertiesList), updateResult, newProperty, newProperties = {};
            for (var i = 0; i < effects.length; i++) {
	...
	*/
    ...
    19: [
      ...
      },
      {
        ...
        './canvas': 3,
        './effect': 4,
        './effects/fadeEffect': 5,
        './effects/moveEffect': 6,
        './mediaFrame': 16,
        './mediaObject': 17,
        './mediaTimeline': 18,
        './photo': 21,
        ...
        './text': 36,
        './timeline': 37,
        './visualMediaObject': 38
      }
    ],
    20: [
      function (_dereq_, module, exports) {
        'use strict';
        var Common = _dereq_('animates-common'), JsonSerializer = _dereq_('./serialization/jsonSerializer'), PropertyBuilder = _dereq_('./properties/propertyBuilder'), DictionaryPropertyBuilder = _dereq_('./properties/dictionaryPropertyBuilder'), CompositePropertyBuilder = _dereq_('./properties/compositePropertyBuilder'), straightPathStrategy = _dereq_('./effects/pathStrategies/straightPathStrategy'), Effect = _dereq_('./effect.js');
        /**
 ...
 */
        function MultiPointEffect(options, builder, pointsSchemaBuilder) {
          var _self = this, propBuilder, defaultOptions = { 'points': {} }, currentOptions, cachedPointsArray;
          /**
	 *  Constructor
	 */
          (function preInit() {
            ...
            currentOptions = Common.extend(options || {}, defaultOptions);
            propBuilder.property('points', DictionaryPropertyBuilder).instancedSchema(pointsSchemaBuilder).property('tick', PropertyBuilder).type('float').add().add().values(currentOptions.points).add();
            _self.Effect(currentOptions, propBuilder);
          }());
          this.refreshPointsArray = function refreshPointsArray() {
            var pointsArray = [], points = _self.getOption('points');
          ...
          this.setOption = function (name, value) {
            if (name.slice(0, 6) === 'points' && name.slice(-4) === 'tick') {
              ...
              var points = _self.getOption('points'), pointId = name.slice(7).slice(0, -5);
              for (var key in points) {
	...
	*/
      ...
      {
        './effect.js': 4,
        './effects/pathStrategies/straightPathStrategy': 13,
        './properties/compositePropertyBuilder': 23,
        './properties/dictionaryPropertyBuilder': 25,
        './properties/propertyBuilder': 27,
        './serialization/jsonSerializer': 30,
    ...
    21: [
      function (_dereq_, module, exports) {
        ...
        'use strict';
        var VisualMediaObject = _dereq_('./visualMediaObject'), JsonSerializer = _dereq_('./serialization/jsonSerializer'), PropertyBuilder = _dereq_('./properties/propertyBuilder'), CompositePropertyBuilder = _dereq_('./properties/compositePropertyBuilder'), Common = _dereq_('animates-common');
        /**
 *  Creates a new Photo
 *  @class Represents a Photo.
 */
        function Photo(options, builder) {
          var _self = this, propBuilder, properties, defaultOptions = {
              height: 100,
              width: 100,
              source: '',
              name: 'Photo'
	...
	*/
          (function preInit() {
            propBuilder = builder || new CompositePropertyBuilder();
            options = Common.extend(options || {}, defaultOptions);
            propBuilder.property('height', PropertyBuilder).value(options.height).type('float').constraint(function (val) {
              return val >= 0;
            }).add().property('width', PropertyBuilder).value(options.width).type('float').constraint(function (val) {
              return val >= 0;
            }).add().property('source', PropertyBuilder).value(options.source).type('imageFile').add();
	...
	*/
          ...
          this.get = function (name) {
            var parts = name.split('.'), currentPart = '', length = parts.length, property;
	...
	*/
    ...
    23: [
      function (_dereq_, module, exports) {
        'use strict';
        var PropertyBuilder = _dereq_('./propertyBuilder'), DictionaryPropertyBuilder = _dereq_('./dictionaryPropertyBuilder'), PropertyBuilder = _dereq_('./propertyBuilder'), CompositeProperty = _dereq_('./compositeProperty');
        /**
 ...
 */
        function CompositePropertyBuilder() {
          var _self = this, name = '', properties = new CompositeProperty();
          /**
	*  Constructor
	*/
          (function preInit() {
	...
	*/
      ...
      {
        './compositeProperty': 22,
        './dictionaryPropertyBuilder': 25,
        './propertyBuilder': 27
      }
    ],
    24: [
      function (_dereq_, module, exports) {
        'use strict';
        var Common = _dereq_('animates-common'), CompositeProperty = _dereq_('./compositeProperty');
        function DictionaryProperty() {
          var _self = this, schema, base_add;
          /**
	 *  Constructor
	 */
          (function preInit() {
          ...
          this.values = function () {
            var values = {}, names = _self.names(true);
	...
	*/
    ...
    25: [
      function (_dereq_, module, exports) {
        'use strict';
        var PropertyBuilder = _dereq_('./propertyBuilder'), CompositePropertyBuilder = _dereq_('./compositePropertyBuilder'), CompositeProperty = _dereq_('./compositeProperty'), DictionaryProperty = _dereq_('./dictionaryProperty');
        /**
 *  Builds a dictionary of custom properties
 *  @class Represents an DictionaryPropertyBuilder.
 */
        function DictionaryPropertyBuilder() {
          var _self = this, name = '', currentValues = [], schemaBuilder;
          /**
	*  Constructor
	*/
          (function preInit() {
	...
	*/
      ...
      {
        './compositeProperty': 22,
        './compositePropertyBuilder': 23,
        './dictionaryProperty': 24,
        './propertyBuilder': 27
      }
    ],
    26: [
      function (_dereq_, module, exports) {
        'use strict';
        var Common = _dereq_('animates-common'), JsonSerializer = _dereq_('../serialization/jsonSerializer');
        /**
 ...
 */
	...
	*/
	 ...
	 */
          this.isValid = function isValid(value) {
            var valid = true, valueToValidate = arguments.length > 0 ? value : currentOptions.value;
          ...
          this.clone = function clone() {
            var newPropOp = Common.clone(currentOptions), newProp = new Property(newPropOp);
            return newProp;
          };
          /**
	*  Constructor
	*/
    ...
    27: [
      function (_dereq_, module, exports) {
        'use strict';
        var Common = _dereq_('animates-common'), Property = _dereq_('./property'), TypesManager = _dereq_('./typesManager');
        /**
 ...
 */
	...
	*/
    ...
    31: [
      function (_dereq_, module, exports) {
        'use strict';
        var VisualMediaObject = _dereq_('./visualMediaObject'), JsonSerializer = _dereq_('./serialization/jsonSerializer'), CompositePropertyBuilder = _dereq_('./properties/compositePropertyBuilder'), Common = _dereq_('animates-common');
        /**
 *  Creates a new Shape
 *  @class Represents a Shape.
 */
        function Shape(options, builder) {
          var _self = this, propBuilder, properties, defaultOptions = { name: 'Shape' };
          /**
	*  Constructor
	*/
          (function preInit() {
	...
	*/
    ...
    32: [
      function (_dereq_, module, exports) {
        ...
        /*jslint node: true, todo: true, white: true, plusplus:true */
        'use strict';
        var Shape = _dereq_('../shape'), JsonSerializer = _dereq_('../serialization/jsonSerializer'), PropertyBuilder = _dereq_('../properties/propertyBuilder'), CompositePropertyBuilder = _dereq_('../properties/compositePropertyBuilder'), Common = _dereq_('animates-common');
        /**
 ...
 */
	...
	*/
          (function preInit() {
            propBuilder = builder || new CompositePropertyBuilder();
            options = Common.extend(options || {}, defaultOptions);
            propBuilder.property('radius', PropertyBuilder).value(options.radius).type('float').constraint(function (val) {
              return val >= 0;
            }).add();
 ...
 */
	...
	*/
    ...
    35: [
      function (_dereq_, module, exports) {
        'use strict';
        var MediaObject = _dereq_('./mediaObject'), PropertyBuilder = _dereq_('./properties/propertyBuilder'), CompositePropertyBuilder = _dereq_('./properties/compositePropertyBuilder'), Common = _dereq_('animates-common');
        /**
 *  Creates a new Sound
 ...
 */
	 ...
	 */
          (function preInit() {
            propBuilder = builder || new CompositePropertyBuilder();
            options = Common.extend(options || {}, defaultOptions);
            propBuilder.property('volume', PropertyBuilder).value(options.volume).type('integer').add().property('source', PropertyBuilder).value(options.source).type('string').add();
 ...
 */
	...
	*/
          (function preInit() {
            propBuilder = builder || new CompositePropertyBuilder();
            options = Common.extend(options || {}, defaultOptions);
            propBuilder.property('fontSize', PropertyBuilder).value(options.fontSize).type('integer').add().property('fontWeight', PropertyBuilder).value(options.fontWeight).type('string').strictValues([
              'normal',
              'bold'
            ]).add().property('fontFamily', PropertyBuilder).value(options.fontFamily).type('string').strictValues([
              'Times New Roman',
              'Verdana',
              'Georgia',
              'Arial',
              'Courier New'
            ]).add().property('fontStyle', PropertyBuilder).value(options.fontStyle).type('string').strictValues([
              'normal',
              'italic',
              'oblique'
            ]).add().property('textDecoration', PropertyBuilder).value(options.textDecoration).type('string').strictValues([
              '',
              'underline',
              'overline',
              'line-through'
            ]).add().property('text', PropertyBuilder).value(options.text).type('text').add();
            _self.VisualMediaObject(options, propBuilder);  // Call base constructor
          }());
          this.getScalableProperties = function getScalableProperties() {
            return ['fontSize'];
	...
	*/
    ...
    37: [
      function (_dereq_, module, exports) {
        'use strict';
        var MediaTimeline = _dereq_('./mediaTimeline'), MultiPointMoveEffect = _dereq_('./effects/multiPointMoveEffect'), MultiPointRotateEffect = _dereq_('./effects/multiPointRotateEffect'), MultiPointScaleEffect = _dereq_('./effects/multiPointScaleEffect'), MultiPointRadiusScaleEffect = _dereq_('./effects/multiPointRadiusScaleEffect'), MultiPointFontSizeScaleEffect = _dereq_('./effects/multiPointFontSizeScaleEffect'), MultiPointWidthAndHeightScaleEffect = _dereq_('./effects/multiPointWidthAndHeightScaleEffect'), JsonSerializer = _dereq_('./serialization/jsonSerializer');
        /**
 *  Creates a new Timeline
 *  @class Represents a Timeline.
 */
        function Timeline(options) {
          var _self = this, mediaTimelineCollection = [];
          /**
	*  Constructor
	*/
          (function preInit() {
          ...
          function addDefaultRotateEffect(mediaTimeline, mediaObject) {
            ...
            defaultRotateEffect.updateProperties(0, { 'angle': mediaObject.getProperty('angle') });
          ...
          function addDefaultScaleEffect(mediaTimeline, mediaObject) {
            var scalableProperties = mediaObject.getScalableProperties && mediaObject.getScalableProperties(), scalableData = {};
            if (scalableProperties) {
	 ...
	 */
          this.addMediaObject = function addMediaObject(mediaObject) {
            ...
            if (mediaObject !== undefined) {
              var mediaTimeline, i, mediaObjectId = mediaObject.getGuid();
	 ...
	 */
          ...
          this.toJSON = function toJSON() {
            var ser = { 'mediaTimelines': JsonSerializer.serializeArray(mediaTimelineCollection) };
            return ser;
          };
	 ...
	 */
      ...
      {
        './effects/multiPointFontSizeScaleEffect': 7,
        './effects/multiPointMoveEffect': 8,
        './effects/multiPointRadiusScaleEffect': 9,
        './effects/multiPointRotateEffect': 10,
        './effects/multiPointScaleEffect': 11,
        './effects/multiPointWidthAndHeightScaleEffect': 12,
        './mediaTimeline': 18,
        './serialization/jsonSerializer': 30
      }
    ],
 ...
 */
	 ...
	 */
          (function preInit() {
            propBuilder = builder || new CompositePropertyBuilder();
            options = Common.extend(options || {}, defaultOptions);
            propBuilder.property('opacity', PropertyBuilder).value(options.opacity).type('float').constraint(function (val) {
              return val >= 0 && val <= 1;
            }).add().property('angle', PropertyBuilder).value(options.angle).type('float').constraint(function (val) {
              return val >= 0 && val <= 360;
            }).add().property('fill', PropertyBuilder).value(options.fill).type('color').add().property('position', CompositePropertyBuilder).property('x', PropertyBuilder).type('float').value(options.position.x).add().property('y', PropertyBuilder).type('float').value(options.position.y).add().property('z', PropertyBuilder).type('float').constraint(function (val) {
              return val >= 0;
            }).value(options.position.z).add().add().property('border', CompositePropertyBuilder).property('type', PropertyBuilder).type('string').strictValues([
              'none',
              'dotted',
              'dashed',
              'solid'
            ]).value(options.border.type).add().property('width', PropertyBuilder).type('float').value(options.border.width).add().property('color', PropertyBuilder).type('color').value(options.border.color).add().add();
            _self.MediaObject(options, propBuilder);  // Call base constructor
          }());
